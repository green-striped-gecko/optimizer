...
nrep = 30  #number of repeats for each sampling regime
nrepo =nrep
...
simpops <- run.popgensim(simpops, steps=200, cost.mat, n.offspring=para$n.offspring, n.ind=para$n.ind,
...
A.act <- table(values(landscape))[1]/2500
  ...
  res[[length(res)+1]] <- list(parameters=para, sddetour=sddetour,mantel=result, vanstrien =resvan,mmrr = resmmrr, type="systematic", A.act=A.act, landscape=landscape)
...
# Define x and y locations
para$locs <- NULL
	while(is.null(para$locs)) para$locs <-createpops(n=para$n.pops, mindist = para$mindist, landscape = landscape, plot = FALSE)
...
res[[length(res)+1]] <- list(parameters=para, sddetour=sddetour, mantel=result, vanstrien =resvan,mmrr = resmmrr,  type="random", A.act=A.act, landscape=landscape)
